...
const backupPath = path.join(__dirname, 'Bim.Client', 'src', 'pages', 'settings', 'Settings.backup.jsx');

try {
  // Sauvegarder l'original s'il n'y a pas déjà une sauvegarde
  if (!fs.existsSync(backupPath)) {
    fs.copyFileSync(filePath, backupPath);
    console.log('Fichier de sauvegarde créé:', backupPath);
  }

  // Version corrigée de Settings.jsx
  const fixedContent = `// filepath: c:\\Users\\Salah-Eddine\\BimRecovery\\Bim.Client\\src\\pages\\settings\\Settings.jsx
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      if (updatedUserResponse.ok) {
        ...
        // En mode simulation, mettre à jour l'interface avec l'URL temporaire
        setUserData({
          ...updatedData,
          profilePhoto: imageUrl // Ceci est temporaire et disparaîtra au rechargement
        });
      }
...
`;
  ...
  // Écrire la version fixée dans le fichier
  fs.writeFileSync(filePath, fixedContent, 'utf8');
  
  console.log('Le fichier Settings.jsx a été réparé avec succès !');
} catch (error) {
  console.error('Erreur lors de la réparation du fichier:', error);
}
